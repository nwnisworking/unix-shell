...
// very small editor: ← → move cursor, ↑/↓ history, Backspace, Enter
int readLine(char *out, size_t cap, const char *prompt){
    if (!isatty(STDIN_FILENO)) {
        // fallback to fgets for non-tty
    ...
    for(;;){
        ...
        if(c=='\r' || c=='\n'){
            ...
            if(cur>0){
                ...
                cur--; len--;
                // redraw
                printf("\r%s %s", prompt, out);
                printf("\r\033[%zuC", strlen(prompt)+1+cur); // move cursor
                fflush(stdout);
            }
        }
            ...
            if(len+1 < cap){
                memmove(out+cur+1, out+cur, len-cur);
                out[cur]=c; len++; cur++;
                // redraw current line from cursor
                printf("\r%s %s\033[K", prompt, out);
                printf("\r\033[%zuC", strlen(prompt)+1+cur);
                fflush(stdout);
            }
